...
#		print DATA $file.",data,".$dates[$i].",".$macd[$i].",".$macdsignal[$i]."\n";
		...
		if ( $lasthisto > 0 and $histo < 0) {
			...
			if ( 0 == $money and $i < $#closeprices ) {
				$money = $shares * $closeprices[$i+1] - $brokerfee ;
				$shares = 0;
			}
		}
		if ( $lasthisto < 0 and $histo > 0) {
			print SIGNALS $file.",buy,".$dates[$i]."\n";
			if ( 0 == $shares and $i < $#closeprices ) {
				$shares = ( $money - $brokerfee ) / $closeprices[$i+1] ;
				$money = 0;
			}
		}
	}
	if ( 0 == $money ) {
		print MONEY $file.",shares,".($shares*$closeprices[$#closeprices])."\n";
	} else {
		print MONEY $file.",money,".$money."\n";
	}
}
close SIGNALS;
close DATA;

...
# This script is to be executed nightly until the end of the WAN replacement project.
# This script is part of a two-script bundle which does the following:
###
# Collect script (this script) refreshes the configuration file in "configs" folder for a list of routers in "routers.txt" file from Device Expert
# as long as the router is 2800 or 3800 series router
###
# Check script (the other script), when called against a router name:
# - retrieves latest config from Device Expert
# - checks if the router is a new model
# - if so, then checks if the configuration on the new model is consistent with the old 2800/3800 model and alerts if different:
# Three things to check:
# ET4L - IP on loopback interface
# BW upgrade - check policy on the WAN interface
# subnet expansion - check that routes and interface masks are the same
